...
void* dispDWT(vector<double> &output,vector<vector<double> > &dwtdisp, vector<int> &length , vector<int> &length2, int J) {
        ...
        for (int iter =0; iter < J; iter++) {
			...
			int rows_x = length2[2*iter];//只考慮down sample
			int cols_x =length2[2*iter +1];
			//length=[floor( (圖片大小 + lf -1)/2  )   圖片大小] 
			//lenght2=[ceil(down(圖片大小)) 原圖大小基數補一行]
			//d_cols=上面相減
			...
			int d_rows = length[2 * iter] - length2[2 * iter];
			int d_cols = length[2 * iter + 1] - length2[2 * iter + 1];
			int d_cols2 = (int) ceil( (double) (d_cols - 1) / 2.0);//相差半徑
			...
			if (iter ==0) {
				for(int i =0; i < rows_x; i++){
					for (int j =0; j < cols_x; j++){
							...
							if (iter == 0) {
									sum+= 4*rows_n*cols_n;//1234 band
							}
							else {
									sum+= 3*rows_n * cols_n;//234
							}
...
//2
void* dwt2_sym(string name,vector<vector<double> > &signal,
	vector<vector<double> >  &cLL, vector<vector<double> >  &cLH, vector<vector<double> >  &cHL, vector<vector<double> > &cHH){
...
//Analysis
    ...
    // 1.Implementing row filtering and column downsampling in each branch.
    for (int i =0; i < rows; i++) {
				...
				//cout <<"一維的長度"<< temp_row.size()<<endl<<lp1.size()<<endl;


				//floor(len/2)
                dwt1_sym_m(name,temp_row,oup_lp,oup_hp);///原始圖片拉成一條一條x再convo////////////////////////////////////////////////////////////////////////



				//輸出為1Dvec
				//cout << temp_row.size() << "  " << oup_hp.size() << " " << oup_lp.size()<<endl;
    ...
    cols =cols_lp1;
    // 2. Implementing column filtering and row downsampling in Low Pass branch.
    ...
    //3. Implementing column filtering and row downsampling in High Pass branch.
...
void* dwt_2d_sym(vector<vector<double> > &origsig, int J, string nm, vector<double> &dwt_output
   ...
   vector<vector<double> >  sig = origsig;
    int rows_n = sig.size(); // y=?
    int cols_n = sig[0].size(); //x=?
    vector<vector<double> > original_copy(rows_n,vector<double>(cols_n));

        original_copy = sig;

                   
        flag.push_back(double(J));//flag[0]=轉換次數
			//length={y長度   x長度}
            ...
            length.insert(length.begin(),rows_n);
				...
				vector<double> lp1, hp1, lp2, hp2;
				filtcoef(nm, lp1, hp1, lp2, hp2);
				unsigned int lf = lp1.size();//filter 長度
				...
				//開始分解，並套入1d的轉換公式
                 for (int iter = 0; iter < J; iter++) {
                        ...
                        rows_n =(int) floor((double) (rows_n + lf -1)/2);//convo完的長度，downsample
                        cols_n =(int) floor((double) (cols_n + lf -1)/2);//算出下一層的高寬
                        length.insert(length.begin(),cols_n);
                        length.insert(length.begin(),rows_n);
						cout << length[0] << " " << length[1] << endl;
						//length={縮小Y 縮小X y長度   x長度}
								...
								//2
								//先除以2只是為了要把變數大小找出來//////////////////////////////計算
                                dwt2_sym(nm,original_copy,cA,cH,cV,cD);/////////////////////////////////////////////////////////important
                                vector<double>   temp_sig2;




                    original_copy = cA;//下面把算出來的轉成1d，以z的形狀紀錄
                    if (iter == J-1) {
                                for(int i =0; i < rows_n; i++){
                                        for (int j =0; j < cols_n; j++){
                                        double temp=cA[i][j];
                                ...
                                for(int i =0; i < rows_n; i++){
                                        for (int j = cols_n; j < cols_n * 2; j++){
                                        double temp =cH[i][j - cols_n];//因為結果分開存再4個小矩陣所以要減掉x長
																	   //for迴圈(大張的圖片)跑遍四張小圖
                                        temp_sig2.push_back(temp);
                                        }
                                }
                                ...
                                sum_coef += 4 * rows_n * cols_n;//算爽的?  sum_coef
...
//頭尾對稱增長n個，n為filter 長度，以對稱的方式增加   cba ||abcdefg|| gfe
void* symm_ext(vector<double> &sig, int a) {
        unsigned int len = sig.size();
        for (int i =0; i < a; i++) {//a是filter長度
                double temp1= sig[i * 2];
                double temp2= sig[len - 1];//訊號長度減1，就是最後一個
...
void* dwt1_sym_m(string wname, vector<double> &signal, vector<double> &cA, vector<double> &cD) {
        ...
        vector<double> lp1, hp1, lp2, hp2;
            ...
            int D = 2; // Downsampling Factor is 2
            int lf = lp1.size();
			//頭尾對稱增長n-1個，n為filter 長度，convolution保持在運算範圍內的內積點數相等
			//運算範圍外不固定的要切掉
		//	cout << "訊號長(x長)"<<signal.size() << endl<< signal.size()+lf-1;
         
			//1.
			vector<double> cA_undec, ddf;
			convfftm(signal, lp1, ddf);
			symm_ext(signal,lf-1);//cba ||abcdefg|| gfe
			//cout << "訊號長(x長)" << signal.size() << endl << signal.size() + lf - 1;
               // vector<double> cA_undec,ddf;
                    //2.LP
				
                    convfftm(signal,lp1,cA_undec);
					//cout << "out" << ddf.size() << endl;
					//cout << "out" << cA_undec.size() << endl;
                    cA_undec.erase(cA_undec.begin(),cA_undec.begin()+lf);
					//cout << "cut1" << cA_undec.size() << endl;
                    cA_undec.erase(cA_undec.end()-lf+1,cA_undec.end());
					//cout << "cut2" << cA_undec.size() << endl;
                    downsamp(cA_undec, D, cA);//這裡會直接切掉，只取整數部分，這裡取偶數135，不是0246，因為前面多切一點
					//  cA.erase(cA.begin(),cA.begin()+(int) ceil(((double)lf-1.0)/2.0));
					//  cA.erase(cA.end()-(int) ceil(((double)lf-1.0)/2.0),cA.end());
					//cout <<"down1"<< cA.size() << endl;
					//結論:5點-->兩點，4-->2，用ceil在downsample上只是為了取基數點。
                    //HP
					//symm_ext(signal,lf-1);//cba ||abcdefg|| gfe
                    vector<double> cD_undec;
                    ...
                    cD_undec.erase(cD_undec.end()-lf+1,cD_undec.end());
                    downsamp(cD_undec,D,cD);
					//cout << "down2" << cD.size() << endl;
					//cout <<"end長"<< cD.size() << endl;
                  //   cD.erase(cD.begin(),cD.begin()+(int) ceil(((double)lf-1.0)/2.0));
                  //   cD.erase(cD.end()-(int) ceil(((double)lf-1.0)/2.0),cD.end());
  return 0;
}
...
void* dwt_sym(vector<double> &signal, int J,string nm, vector<double> &dwt_output,vector<double> &flag, vector<int> &length){
    ...
    unsigned int temp_len = signal.size();
        if ( (temp_len % 2) != 0) {// 如果長度是奇數   長度要改成偶數
                double temp =signal[temp_len - 1];//改的方法用sym對稱  | 123 |321
                signal.push_back(temp);
                flag.push_back(1);//是奇數第一個bit設1
                temp_len++;
        } else {
                flag.push_back(0);//flag[0]=0(偶數)
        }
        length.push_back(temp_len);
        flag.push_back(double(J));
        // flag[2] contains symmetric extension length
		//flag 有兩個屬性   想成flag[2]={改成偶數長度後的訊號長度, 做幾次小波分解}

    vector<double> original_copy, appx_sig, det_sig;//approximate   detail
    original_copy = signal;
        ...
        for (int iter = 0; iter < J; iter++) {
			//dwt_sym(vector<double> &signal, int J,string nm, vector<double> &dwt_output,vector<double> &flag, vector<int> &length){
            dwt1_sym(nm,signal, appx_sig, det_sig);

			//把算出來的detail signal 插入dwt_out的最前面
            dwt_output.insert(dwt_output.begin(),det_sig.begin(),det_sig.end());
            int l_temp = det_sig.size();//detail signal 的長度
            length.insert(length.begin(),l_temp);//把detail signal 的長度插入length的最前面

			//算到最後一次要把它組裝起來，要把approximate訊號存到output，並把長度寫到length
			//忘記的話想成小方波拆解成大方波和大的正反方波
            if (iter == J-1 ) {
			...
			//為下一層做計算 把signal設為上一層的approximate訊號
			//其他的清空
            signal.clear();
            signal = appx_sig;
            appx_sig.clear();
...
double convfft(vector<double> &a, vector<double> &b, vector<double> &c) {//a*b=c
...
int filtcoef(string name, vector<double> &lp1, vector<double> &hp1, vector<double> &lp2,
    ...
    if (name == "haar" || name == "db1" ) {
       /* lp1.push_back(0.7071);lp1.push_back(0.7071);
		...
		lp1.push_back(0.5); lp1.push_back(0.5);
		hp1.push_back(-0.5); hp1.push_back(0.5);
		lp2.push_back(0.5); lp2.push_back(0.5);
		hp2.push_back(0.5); hp2.push_back(-0.5);
  //  	cout << lp2[1] << endl;
//    	hpd = {-0.7071, 0.7071};
//    	lpr = {0.7071, 0.7071};
//    	hpr = {0.7071, -0.7071};
